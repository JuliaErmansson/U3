...
function renderCourse (course) {
  ...
  div.id = 'container'
  div.innerHTML = `<h1>${course.title} (${course.totalCredits} credits)</h1>
...
function courseRes (id) {
  ...
  let resName = allTheTeachers.map(
    teacher =>
...
function allStudents (thisCourse) {
  let allCourseStudents = allTheStudents.filter(student =>
    student.courses.some(course => course.courseId == thisCourse.courseId)
  )
  let theStudents = []
  let div = document.createElement('div')
  for (let student of allCourseStudents) {
    let idGetCourse = student.courses.filter(
      course => course.courseId == thisCourse.courseId
    )
